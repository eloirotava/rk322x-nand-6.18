...
static int nand_add_dev(struct nand_blk_ops *nand_ops, struct nand_part *part)
{
	...
	/*
	 * 6.10+: limits are passed into blk_mq_alloc_disk(). The old
	 * blk_queue_max_* helpers are gone.
	 */
	{
		...
		gd = blk_mq_alloc_disk(nand_ops->tag_set, &lim, dev);
	}
	if (IS_ERR(gd)) {
		ret = PTR_ERR(gd);
		kfree(dev);
...
static int nand_blk_register(struct nand_blk_ops *nand_ops)
{
	...
	nand_ops->tag_set->flags = BLK_MQ_F_BLOCKING;
